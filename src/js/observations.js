...
function titleCase(s) {
  var firstUpper = s.toLowerCase().split(" ");
  for (var i = 0; i < firstUpper.length; i++) {
    firstUpper[i] =
      firstUpper[i].charAt(0).toUpperCase() + firstUpper[i].substring(1);
  }
...
function transformObservations(observations) {
  return observations.map(function (observation) {
    ...
    obj.date = new Date(Date.parse(observation.created_at));
